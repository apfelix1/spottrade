...
class DT:
    ...
    def get_stock_TAQ_array(self):
        ...
        for i in tradelist[:,0]:
            ...
            while etfindex <rtarr.shape[0]:
                ...
                etfindex += 1
            setattr(DT, i + 'stk_i_s10', stockTAQ.columns.values.tolist().index('SellVolume10'))
            setattr(DT, i + 'stk_i_b10', stockTAQ.columns.values.tolist().index('BuyVolume10'))
            setattr(DT, i + 'stk_i_s1', stockTAQ.columns.values.tolist().index('SellVolume01'))
            setattr(DT, i + 'stk_i_b1', stockTAQ.columns.values.tolist().index('BuyVolume01'))
            setattr(DT, i + 'stk_i_sp10', stockTAQ.columns.values.tolist().index('SellPrice10'))
            setattr(DT, i + 'stk_i_bp10', stockTAQ.columns.values.tolist().index('BuyPrice10'))
            setattr(DT, i + 'stk_i_sp1', stockTAQ.columns.values.tolist().index('SellPrice01'))
            setattr(DT, i + 'stk_i_bp1', stockTAQ.columns.values.tolist().index('BuyPrice01'))

            setattr(DT, namearr, adjstockArr)
    ...
    def get_stock_buy_cost(self, stockNumber, quant):
        ...
        # get crucial index
        i_s10 = getattr(DT, stockNumber + 'stk_i_s10')
        i_s1 = getattr(DT, stockNumber + 'stk_i_s1')
        i_sp10 = getattr(DT, stockNumber + 'stk_i_sp10')
        i_sp1 = getattr(DT, stockNumber + 'stk_i_sp1')

        stocktaq[:, -3] = stocktaq[:, i_s10:i_s1 + 1].astype(np.float).sum(axis=1)
        ...
        while i < 10:
            ...
            can_trade[:, -2] = can_trade[:, -2].astype(np.float) + can_trade[:, i_sp1 - i].astype(np.float) * minrow
            i += 1
    ...
    def get_stock_sell_return(self, stockNumber, quant):
        ...
        # get crucial index
        i_b10 = getattr(DT, stockNumber + 'stk_i_b10')
        i_b1 = getattr(DT, stockNumber + 'stk_i_b1')
        i_bp10 = getattr(DT, stockNumber + 'stk_i_bp10')
        i_bp1 = getattr(DT, stockNumber + 'stk_i_bp1')
        #  sum vol
        stocktaq[:, -3] = stocktaq[:, i_b1:i_b10 + 1].astype(np.float).sum(axis=1)
        ...
        while i < 10:
            ...
            can_trade[:, -4] = can_trade[:, -4].astype(np.float) + minrow
            # update total cost
            can_trade[:, -2] = can_trade[:, -2].astype(np.float) + can_trade[:, i_bp1 + i].astype(np.float) * minrow
            i += 1
        stocktaq[stocktaq[:, -3].astype(np.float) >= quant] = can_trade
    ...
    def get_discount_IOPV(self):
        ...
        while i < float(tradelist.shape[0]):
            if float(tradelist[i, 3]) == 2 or float(tradelist[i, 3]) == 4:
                ...
                self.get_stock_buy_cost(stockNumber, stockQuant)
                i += 1
        self.rtarr[:, 4] = (self.stktdarr[:, 3].astype(np.float) + self.cash_component) * self.stktdarr[:, 2].astype(
            np.float)
    ...
    def get_dc_rate(self):
        ...
        rtarr[rtarr[:,2].astype(np.float) * rtarr[:,4].astye(np.float) > 0] = rtarr_dc
