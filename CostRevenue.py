...
class DT:
    ...
    secUtils = CSecurityMarketDataUtils('Z:/StockData', 'C:/GTALocalData')
...
apr_tdPeriodList = TradingDays(startDate='20200401', endDate='20200404')
...
for i in apr_tdPeriodList:
    dTypes = ['TAQ']
    i = i.replace("-", "")
    secUtils.ExtractGTAData(i, 'STOCK', dTypes)
    secUtils.ExtractGTAData(i, 'FUND', dTypes)

    a = DT('510300.SH', i)
    apr_max_rate.append(a.get_max_rate())
    apr_mean_rate.append(a.get_mean_rate())
    secUtils.ClearGTAData(i, 'STOCK', dTypes)
    secUtils.ClearGTAData(i, 'FUND', dTypes)
